...
dumpLog($offset, '1', 'update_bid_ttpomodel.log');
...
if (!isset($campaigns["error"]['error_code'])) {
    ...
    foreach ($campaigns["result"]["Campaigns"] as $num => $dat) {
        ...
        /*
         * Первое место + 25% но не более 330руб.
         *
         * Исключение - РСЯ, эти компании полностью игнорируются
         * */
        // Filter
        $arNameWords = explode(" ", $dat["Name"]);
        //if (!in_array("РСЯ", $arNameWords) && (in_array("(msk)", $arNameWords) || in_array("(spb)", $arNameWords))) {
        if (!in_array("РСЯ", $arNameWords)) {
            $arrBidsParam["auto"][] = array(
                'Scope' => array('SEARCH'),
                'CampaignId' => $dat["Id"],
                "IncreasePercent" => 25, // (это проценты)
                "CalculateBy" => "VALUE",
                "Position" => "P11",
    ...
    foreach ($arrBidsParam as $type => $val) {
        ...
        error_log($campaignsInfo['info'][5], 3, dirname(__FILE__) . "/log/" . $markOffset[$offset] . "/" . date("d.m.Y") . ".log");
