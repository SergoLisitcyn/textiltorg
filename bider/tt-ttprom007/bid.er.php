...
$account_data = array();
$account_data['login'] = 'tt-ttprom';
$account_data['token'] = 'AgAAAAAg8tyAAAXaF7nIqR6T_01yph9LN0izDZw';
...
dumpLog($offset, '1', 'tt-ttprom007.log');
...
if (!isset($campaigns["error"]['error_code'])) {
    ...
    foreach ($campaigns["result"]["Campaigns"] as $num => $dat) {
        ...
        // Третье место + 25%, не более 330р (спец-размещение)
        $arrBidsParam[] = array(
            'Scope' => array('SEARCH'),
            'CampaignId' => $dat["Id"],
