;(function (window)
{
	...
	MessengerCommon.prototype.formatDate = function(timestamp, format)
	{
	...
	MessengerCommon.prototype.getDateDiff = function (timestamp)
	{
	...
	MessengerCommon.prototype.hideErrorImage = function(element)
	{
	...
	MessengerCommon.prototype.purifyText = function(text)
	{
	...
	MessengerCommon.prototype.getUserStatus = function(userId, getText)
	{
		...
		var userData = typeof(this.BXIM.messenger.users[userId]) == 'undefined'? false: this.BXIM.messenger.users[userId];
		...
		else if (userData.network || userData.bot && this.BXIM.messenger.bot[userId] && this.BXIM.messenger.bot[userId].type == 'network')
		{
			status = 'network';
		...
		else if (userData.birthday == BX.date.format('d-m') && (userData.status == 'online' || !online.isOnline))
		{
			status = 'birthday';
			if (online.isOnline)
			{
		...
		else if (this.getUserIdleStatus(userId, online))
		{
			status = 'idle';
	...
	MessengerCommon.prototype.getOnlineData = function(userData)
	{
		var online = {};
		if (userData)
		{
			if (userData.id == this.BXIM.userId)
			{
				userData.lastActivityDate = parseInt(new Date().getTime()/1000);
				userData.mobileLastDate = 0;
				userData.idle = 0;
				userData.status = this.BXIM.settings.status;
			}

			online = BX.user.getOnlineStatus(parseInt(userData.lastActivityDate));
		}

		return online;
	};

	MessengerCommon.prototype.getUserIdle = function(userId)
	{
		userId = parseInt(userId);
		userId = isNaN(userId)? this.BXIM.userId: userId;
	...
	MessengerCommon.prototype.getUserMobileStatus = function(userId)
	{
		userId = parseInt(userId);
		userId = isNaN(userId)? this.BXIM.userId: userId;
		...
		var lastSeen = parseInt(this.BXIM.messenger.users[userId].lastActivityDate);
		if (
			mobileLastDate
			&& this.getNowDate()-parseInt(mobileLastDate) < BX.user.getSecondsForLimitOnline()
			&& lastSeen - parseInt(mobileLastDate) < 300
		)
		{
			status = true;
		}
	...
	MessengerCommon.prototype.getUserIdleStatus = function(userId, online)
	{
		userId = parseInt(userId);
		userId = isNaN(userId)? this.BXIM.userId: userId;
		online = online? online: BX.user.getOnlineStatus(parseInt(this.BXIM.messenger.users[userId].lastActivityDate));
	...
	MessengerCommon.prototype.getUserPosition = function(userId, recent)
	{
		recent = recent === true;
		...
		if (recent && parseInt(this.BXIM.messenger.users[userId].lastActivityDate) && !(this.BXIM.messenger.users[userId].bot || this.BXIM.messenger.users[userId].network))
		{
			return this.getUserLastDate(userId);
		}

		else if(this.BXIM.messenger.users[userId].workPosition)
		{
			pos= this.BXIM.messenger.users[userId].workPosition;
		}
		else if (this.BXIM.messenger.users[userId].extranet || this.BXIM.messenger.users[userId].network)
		{
		...
		{
			pos = this.BXIM.bitrixIntranet? BX.message('IM_CL_USER'): BX.message('IM_CL_USER_B24');
		}

		return pos
	}

	MessengerCommon.prototype.getUserLastDate = function(userId)
	{
		if (!this.BXIM.messenger.users[userId])
			return '';

		var pos = '';
		if (this.BXIM.messenger.users[userId].bot || this.BXIM.messenger.users[userId].network)
		{
			pos = '';
		}
		else if (this.BXIM.messenger.users[userId].isAbsent)
		{
			var online = this.getOnlineData(this.BXIM.messenger.users[userId]);
			var absentDate = new Date(this.BXIM.messenger.users[userId].isAbsent);
			absentDate = BX.date.format(BX.date.convertBitrixFormat(BX.message("FORMAT_DATE")), absentDate.getTime()/1000);
			pos = BX.message('IM_STATUS_VACATION_TITLE').replace('#DATE#', absentDate);

			if (online.isOnline && parseInt(this.BXIM.messenger.users[userId].idle) > 0)
			{
				 pos = BX.message('IM_STATUS_AWAY_TITLE').replace('#TIME#', this.getUserIdle(userId));
			}
			else if (online.isOnline && !online.lastSeenText)
			{
				pos = BX.message('IM_STATUS_ONLINE')+'. '+pos;
			}
			else if (online.lastSeenText)
			{
				pos = BX.message('IM_LS_'+(this.BXIM.messenger.users[userId].gender == 'F'? 'F': 'M')).replace('#POSITION#', pos).replace('#LAST_SEEN#', online.lastSeenText);
			}
		}
		else if (parseInt(this.BXIM.messenger.users[userId].lastActivityDate))
		{
			var online = this.getOnlineData(this.BXIM.messenger.users[userId]);
			if (online.isOnline && parseInt(this.BXIM.messenger.users[userId].idle) > 0)
			{
			...
			{
				pos = BX.message('IM_STATUS_ONLINE');
			}
			else if (online.lastSeenText)
			{
				pos = BX.message('IM_LS_SHORT_'+(this.BXIM.messenger.users[userId].gender == 'F'? 'F': 'M')).replace('#LAST_SEEN#', online.lastSeenText);
			}
		}
	...
	MessengerCommon.prototype.recentLinesListPrepare = function(params)
	{
		...
		if (this.BXIM.settings.linesTabOrder == 'group' || this.BXIM.settings.linesTabOrder == 'support')
		{
			...
			recentLinesList.sort(BX.delegate(function (i, ii)
			{
				...
				if (priorityIsActive && !chatLinesAssoc[i.chatId])
				{
					var source = this.BXIM.messenger.chat[i.chatId].entity_id.toString().split('|');
					chatLinesAssoc[i.chatId] = linesPriority[source[1]]? linesPriority[source[1]]: 0;
				}

				var i1 = this.BXIM.messenger.chat[i.chatId].entity_data_1.toString().split('|');
				if (typeof(i1[6]) != 'undefined')
				{
					i1 = parseInt(i1[6])-(priorityIsActive? chatLinesAssoc[i.chatId]: 0);
				}
				else
				{
					i1 = typeof(i1[5]) != 'undefined'? parseInt(i1[5]): 0;
				}



				if (priorityIsActive && !chatLinesAssoc[ii.chatId])
				{
					var source = this.BXIM.messenger.chat[ii.chatId].entity_id.toString().split('|');
					chatLinesAssoc[ii.chatId] = linesPriority[source[1]]? linesPriority[source[1]]: 0;
				}

				var i2 = this.BXIM.messenger.chat[ii.chatId].entity_data_1.toString().split('|');
				if (typeof(i2[6]) != 'undefined')
				{
					i2 = parseInt(i2[6])-(priorityIsActive? chatLinesAssoc[ii.chatId]: 0);
				}
				else
				{
					i2 = typeof(i2[5]) != 'undefined'? parseInt(i2[5]): 0;
				}
	...
	MessengerCommon.prototype.recentListHide = function(userId, sendAjax)
	{
		if (!userId)
			return false;
	...
	MessengerCommon.prototype.recentListElementToTop = function(userId)
	{
		...
		if (!userFound)
		{
			...
			if (!messageText)
			{
				...
				{
					messageText = BX.util.htmlspecialcharsback(this.getUserPosition(userId, true));
				}
			}
	...
	MessengerCommon.prototype.drawContactListElement = function(params)
	{
		...
		else
		{
			...
			{
				description = this.getUserPosition(params.id, true);
			}
		}
						...
						BX.create("span", { props : { className : "bx-messenger-content-item-text-center"}, children: [
							BX.create("span", {  props : { className : "bx-messenger-content-item-text-message"}, children: [
								BX.create("span", {  props : { className : "bx-messenger-content-item-text-wrap"+(appendTop? " bx-messenger-content-item-text-wrap-append": "")}, children: [
									BX.create("span", { props : { className : "bx-messenger-content-item-reply"}, attrs: {'data-messageId': message.id, title : BX.message('IM_M_REPLY_TITLE')}}),
				...
				var arMessage = BX.create("div", { attrs : { 'data-type': 'other', 'data-senderId' : message.senderId, 'data-messageDate' : message.date, 'data-messageId' : message.id, 'data-blockmessageid' : message.id }, props: { className : "bx-messenger-content-item bx-messenger-content-item-2"+(markNewMessage? ' bx-messenger-content-item-new': '')+" "+extraClass}, children: [
					...
					BX.create("span", { props : { className : "bx-messenger-content-item-content"}, children : [
						...
						BX.create("span", { props : { className : "bx-messenger-content-item-text-center"}, children: [
							BX.create("span", {  props : { className : "bx-messenger-content-item-text-message"}, children: [
								BX.create("span", {  props : { className : "bx-messenger-content-item-text-wrap"+(appendTop? " bx-messenger-content-item-text-wrap-append": "")}, children: [
									BX.create("span", { attrs: {title : BX.message('IM_M_REPLY_TITLE')}, props : { className : "bx-messenger-content-item-reply", title : BX.message('IM_M_REPLY_TITLE'), 'data-messageId': message.id}}),
									BX.create("span", { attrs: {title : (withAppsMenu? BX.message('IM_M_MENU_APP_EXISTS')+' ': '')+BX.message('IM_M_OPEN_EXTRA_TITLE').replace('#SHORTCUT#', BX.browser.IsMac()?'CMD':'CTRL')}, props : { className : "bx-messenger-content-item-menu"+(withAppsMenu? ' bx-messenger-content-item-menu-with-apps': '')}}),
									textNode, filesNode, attachNode
								]})
							]}),
	...
	MessengerCommon.prototype.pullEvent = function()
	{
		var pullHandler = BX.delegate(function(command,params,extra)
		{
			...
			else if (command == 'readMessageOpponent')
			{
				...
				{
					this.BXIM.messenger.users[params.userId].idle = 0;
					this.BXIM.messenger.users[params.userId].lastActivityDate = parseInt(new Date().getTime()/1000);

					if (this.BXIM.messenger.currentTab.toString() == params.userId.toString())
					{
						var getLastDate = BX.MessengerCommon.getUserLastDate(params.userId);
						if (this.isMobile())
						{
				...
				if (this.isBot(params.userId) && !params.DEFERRED && this.BXIM.messenger.showMessage[params.dialogId] && this.BXIM.messenger.showMessage[params.dialogId].length)
				{
					...
					if (bot.type == 'human')
					{
						...
						setTimeout(BX.delegate(function(){
							...
							BX.onCustomEvent(window, (this.isMobile()? "onPull-im": "onPullEvent-im"), [deferredPull.command, deferredPull.params, deferredPull.extra]);
						}, this), 1000);
		...
		var pullOnlineHandler = BX.delegate(function(command, params)
		{
			...
			else if (false && command == 'user_offline')
			{
			...
			else if (command == 'list' || command == 'userStatus')
			{
				var contactListRedraw = false;
				...
				for (var i in params.users)
				{
					...
					this.BXIM.messenger.users[i].lastActivityDate = params.users[i].lastActivityDate;
				}
	...
	MessengerCommon.prototype.drawTab = function(userId, scroll, messageCount, changeTab)
	{
		...
		if (this.BXIM.messenger.openChatFlag)
		{
			...
			if (this.BXIM.messenger.chat[chatId])
			{
				if (this.BXIM.messenger.chat[chatId].type == 'open' || (this.BXIM.messenger.chat[chatId].type == 'lines' && !BX.MessengerCommon.userInChat(chatId)))
				{
	...
	MessengerCommon.prototype.linesOpenSession = function(userCode)
	{
		BX.ajax({
	...
	MessengerCommon.prototype.linesVoteSend = function(dialogId, messageId, rating)
	{
		...
		if (parseInt(this.BXIM.messenger.message[messageId].date)+86400 < (new Date().getTime())/1000)
		{
	...
	MessengerCommon.prototype.linesGetSession = function(chatId)
	{
		var session = null;
	...
	MessengerCommon.prototype.linesGetSource = function(chatId)
	{
		var sourceId = '';
		if (!this.BXIM.messenger.chat[chatId])
			return sourceId;
