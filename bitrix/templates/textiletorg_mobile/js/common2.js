...
function sh_geo(data){
	if(data)
	{
		...
		}//<a href='/informacija/kredit/'><img src='/_mod_files/ce_images/credit.gif'></a>
		/*if(data['id']==1 || data['id']==2){
				$(".akcii_img").html("<br /><script language='JavaScript' charset='utf-8' src='/_img/js_akc_baner.js'></script>");
		}else
		if(data['id']!=1 && data['id']!=2){
			$(".akcii_img").html("<script language='JavaScript' charset='utf-8' src='/_img/js_akc_baner.js'></script>");
		}*/
	}
}
...
function show_hide_geo(sh){
	if(sh==0){
		$('.popup__into_overlay').hide();
        $('#win_choose_region').hide();
	} else {
		$('#win_choose_region').css('top', ($(window).scrollTop() + 40) + 'px');
		$('.popup__into_overlay').show();
        $('#win_choose_region').show();
	}
}
...
function getLocalDeliveryPrice(u_id, u_id_city, u_lenght, u_region_id){
	$("#dostavka").prepend('<span class="delivery_loader"><label>Доставка: </label><img src="/_img/load.gif" /></span>');
	$.ajax({
...
function getPecom(data) {
	$("#dostavka_").prepend('<span class="delivery_loader_"><label>Доставка: </label><img src="/_img/load.gif" /></span>');
	$.ajax({
...
function getPecomCalk(data) {
	$(".calk_result").prepend('<span class="calk_loader_"><img src="/_img/load.gif" /></span>');
	$.ajax({
		url: "/calk/pecom.php",
		type: "post",
		data: data,
...
function showCottonInfo(item) {
	...
	var data_array = [];
	data_array["Полиэстр"] = '/_mod_files/cotton_prop_page/poliester.html';
	data_array["Спандекс"] = '/_mod_files/cotton_prop_page/spandex.html';
	data_array["Вискоза"] = '/_mod_files/cotton_prop_page/viskoza.html';
	data_array["Хлопчатобумажная"] = '/_mod_files/cotton_prop_page/xlopchatobuma.html';
	data_array["Лен"] = '/_mod_files/cotton_prop_page/len.html';
...
/* ALL_CITY_LIST */
$( ".listalfabet li:first-child a" ).addClass( "acl_activ" );
...
$(".show_all_city").live("click", function(e) {
...
$(".show_famous_city").live("click", function(e) {
...
render_compare_popup();render_compare_button();
	...
	$('.list-red .tg').on('click', function (e) {
		$( "div", this ).toggle();
	});
