...
<div class="pagi ajax" id="ajax-pagination">
    <div class="loader"></div>
    <? if ($arResult["NavPageCount"] > $arResult["NavPageNomer"]) { ?>
        <div class="show-more" data-href="<?=$arResult["sUrlPath"]?>?QUERY=<?=$_REQUEST["QUERY"]?>&PAGEN_<?=$arResult["NavNum"]?>=<?=($arResult["NavPageNomer"]+1)?>">
            <div class="plus"></div>
            Показать еще
        </div>
    <? } ?>
...
?>
		<span class="<?=($bFirst ? "modern-page-first " : "")?>
			...
			select
			selected myselect"><?=$arResult["nStartPage"]?></span>
<?
