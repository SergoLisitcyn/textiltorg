...
<div id="basket_items_list">
	<div class="bx_ordercart_order_table_container">
		<table id="basket_items" border="0" cellspacing="0">
			<thead>
				<tr>
					<?
					foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
						...
						<?
						elseif ($arHeader["id"] == "PROPERTY_VENDOR_CODE_VALUE"):
						?>
						...
						?>
							<?if ($arHeader["id"] != "PROPERTY_VENDOR_CODE_VALUE"):?>
								<?=$arHeader["name"]; ?>
							<?endif?>
							</td>
					<?
			...
			<tbody>
				...
				?>
					<tr id="<?=$arItem["ID"]?>">
						...
						foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
							...
							?>
								...
								<td class="item">
									...
									<div class="bx_ordercart_properties">
										<p><span>Артикул</span>: <?=$arItem["PROPERTY_VENDOR_CODE_VALUE"]?></p>
									</div>
							...
							?>
								<td class="custom summ">
									...
									echo $arItem["SUM"] . ' <span>руб.</span>';

									if ($arItem["RB_SUM"]):
									?>
										<div class="cart-item-price-rb"><?=$arItem["RB_SUM"]?> <span>руб.</span></div>
									<?
									endif;
									if ($arHeader["id"] == "SUM"):
									?>
						...
						?>
							<td class="control del">
								<?
								if ($bDeleteColumn):
								?>
									<a href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>" class="delete">&nbsp;</a><br />
								<?
								endif;
								if ($bDelayColumn):
								?>
									<a href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delay"])?>">&nbsp;</a>
								<?
								endif;
								?>
							</td>
			...
			<table class="bx_ordercart_order_sum">
				...
				<?endif;?>
					<tr>
						<td class="fwb"></td>
						<td class="fwb">
							Товаров: <strong class="custom_all_count"><?=$arResult["CUSTOM_ALL_COUNT"]?></strong>, на сумму: <big id="allSum_FORMATED"><?=$arResult["CUSTOM_ALL_SUM"]?></big> руб.
							<?if ($arResult["CUSTOM_ALL_SUM_RB"]):?>
								<div class="cart-price-rb"><?=$arResult["CUSTOM_ALL_SUM_RB"]?> <small>руб.</small></div>
							<?endif?>
						</td>
					</tr>
