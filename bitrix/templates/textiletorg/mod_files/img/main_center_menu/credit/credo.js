...
function get_cart() {
    var result = $.post( "confirm_order/kredit.php", function( data ) {
        arrayk = JSON.parse(data);
        console.log(arrayk.base64);
        console.log(arrayk.sign);

        vkredit = new VkreditWidget(1, 144574, {
            order: arrayk.base64,
            sig: arrayk.sign
        });
    ...
    $.post(
            ...
            function(data) {
                ...
                if ( !err_reg.test(data) && data!='0' && data!='') {
                    ...
                    var order = {
                        items: [],
                        partnerId: 'a06b000001s7bJ9AAI',
                        partnerOrderId: 'order_'+Math.floor(Math.random()*1000000)
                    };
                    ...
                    for (i = 0; i < cart.length; i++) {
                        ...
                        table_cart += '</div></td><td style="text-align:center;">' + cart[i].count * cart[i].price + '&nbsp;'+price_info+'</td><td><a onclick="del_cart_item(' + cart[i].id + ', '+cart[i].prop_id+');" class="dela_img"><img src="http://www.test.textiletorg.ru/_img/del.png" height="25" alt="Удалить из корзины"></a></td></tr>';
                        ...
                        if (cart[i]['props'].length > 0)
                        {
							...
							bg_pr = '<img src="/'+cart[i]['props'][j].img+'">';
                                ...
                                table_cart += '</div></td><td style="text-align:center;">' + cart[i]['props'][j].count * cart[i]['props'][j].price + '&nbsp;руб.</td><td><a onclick="del_cart_item(' + cart[i].id + ', '+cart[i]['props'][j].id+');" class="dela_img"><img src="http://www.test.textiletorg.ru/_img/del.png" height="25" alt="Удалить из корзины"></a></td></tr>';
                    ...
                    //var base64 = base64_encode(json);

                    // Секретная строка для формирования подписи (выдается Банком)
                    //var secret = 'textiletorg-secret-s7bJ976d';

                    // Формирование подписи
                    //var sign = signMessage(base64, secret);
                    //$(".sign-data").html(sign);
                    //$(".base64-data").html(base64);

                    //console.log(base64_encode(JSON.stringify(order)));
                    //var wee = CryptoJS.SHA1("Message");
                    //console.log(sign);
                    //table_cart += mixmarket.join(';');

                    table_cart += '</table>' + '<input type="hidden" name="row_count" value="' + row_count + '">' + '</form>';
                    $('#top-cart-total').html(count_item);
                    $('#top-cart-price').html(sum_item);
...
function ajax_buy_item(item_id, prop_id, plus, count, ing)
{
    ...
    ,function(data) {
                    ...
                    if (data_cart != null){
                        ...
                        $('#top-cart-total').html(data_cart.eshop_cart_count);
                        $('#top-cart-price').html(data_cart.eshop_cart_total_plain);
...
function signMessage(message, secretPhrase) {
    ...
    result = CryptoJS.MD5(message) + CryptoJS.SHA1(message);
    for (var i = 0; i < 1102; i++) {
        result = CryptoJS.MD5(result);
    }
