			...
			<table class="data-table">
                ...
                if (!empty($arResult["SHOW_FIELDS"]))
                {
                    foreach ($arResult["SHOW_FIELDS"] as $code => $arProp)
                    {
                        ...
                        if ($showRow)
                        {
                            ?><tr><td><div><?=GetMessage("IBLOCK_FIELD_".$code)?></div></td><?
                            foreach($arResult["ITEMS"] as &$arElement)
                            {
                        ?>
                                <td valign="top">
                        <?
                                ...
                                {
                                    case "NAME":
                                        ?><a class="name" href="<?=$arElement["DETAIL_PAGE_URL"]?>"><?=$arElement[$code]?></a>

                                        <?if($arElement["CAN_BUY"]):?>
                                        <!--noindex--><br /><a class="inyourcart" href="<?=$arElement["BUY_URL"]?>" rel="nofollow"><?=GetMessage("CATALOG_COMPARE_BUY"); ?></a><!--/noindex-->
                ...
                if (!empty($arResult["SHOW_OFFER_FIELDS"]))
                {
                    foreach ($arResult["SHOW_OFFER_FIELDS"] as $code => $arProp)
                    {
                        ...
                        ?>
                        <tr>
                            <td><div><?=GetMessage("IBLOCK_OFFER_FIELD_".$code)?></div></td>
                            <?foreach($arResult["ITEMS"] as &$arElement)
                            {
                            ?>
                            <td>
                ...
                ?>
                <tr>
                    <td><div><?=GetMessage('CATALOG_COMPARE_PRICE');?></div></td>
                    <?
                    foreach ($arResult["ITEMS"] as &$arElement)
                    {
                ...
                <? if (!empty($arResult["SHOW_PROPERTIES"]))
                {
                    foreach ($arResult["SHOW_PROPERTIES"] as $code => $arProperty)
                    {
                        ...
                        if ($showRow)
                        {
                            ?>
                            <tr>
                                <td><div title="<?=$arProperty["NAME"]?>"><?=$arProperty["NAME"]?></div></td>
                                <?foreach($arResult["ITEMS"] as &$arElement)
                                {
                                    ?>
                                    <td>
				...
				<tr>
					...
					<td>
                        <div>
						<a class="delite" onclick="CatalogCompareObj.MakeAjaxAction('<?=CUtil::JSEscape($arElement['~DELETE_URL'])?>'); BX.onCustomEvent(window, 'OnCompareChange');" href="javascript:void(0)"><?=GetMessage("CATALOG_REMOVE_PRODUCT")?></a>
                        </div>
                    </td>
					<?
					}
