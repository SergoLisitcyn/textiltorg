    ...
    if ($arItem["PROPERTIES"]["PHOTOS"]["VALUE"])
    {
        ...
        foreach ($arItem["PROPERTIES"]["PHOTOS"]["VALUE"] as $photo)
            $arItem["PHOTOS"][] = array(
				...
				"BIG" => Helper::Resize(array($photo), 800, 600),
            );
    }
...
$db_list = CIBlockSection::GetList(Array(), Array('IBLOCK_ID' => $arParams["IBLOCK_ID"], "ID" => $arParams["SECTION_ID"]), false, array("IBLOCK_ID", "ID", "UF_YM_ID", "UF_TEXT_FOR_FEEDBACK", "UF_TEXT_FOR_TOPSELL", "UF_TEXT_FOR_BOTTSELL"));
while($ar_result = $db_list->GetNext())
{
	$arResult["UF_YM_ID"] = $ar_result["UF_YM_ID"];
	$arResult["UF_TEXT_FOR_FEEDBACK"] = $ar_result["UF_TEXT_FOR_FEEDBACK"];
	$arResult["UF_TEXT_FOR_TOPSELL"] = $ar_result["UF_TEXT_FOR_TOPSELL"];
    $arResult["UF_TEXT_FOR_BOTTSELL"] = $ar_result["UF_TEXT_FOR_BOTTSELL"];
}
...
$arResult["IS_HIDE_DESC"] = true;

// test drive
foreach ($arResult["ITEMS"] as &$arItem)
{
	$arItem["TEST_DRIVE_ITEM"] = null;
	if($arItem["PROPERTIES"]["TEST_DRIVE"]["VALUE"])
	{
		$res = CIBlockElement::GetByID($arItem["PROPERTIES"]["TEST_DRIVE"]["VALUE"]);
		if($arDrive = $res->GetNext())
		{
			$arItem["TEST_DRIVE_ITEM"] = $arDrive;
		}
	}
}
