	...
	<form method="post" action="<?=htmlspecialcharsbx($arResult['SUBMIT_FORM_URL'])?>" enctype="multipart/form-data">
		...
		<div data-role="letter-editor" class="bx-sender-letter-step-2 <?=($arResult['SHOW_TEMPLATE_SELECTOR'] ? 'bx-sender-letter-hide' : 'bx-sender-letter-show')?>">
			...
			<div class="bx-sender-letter-field">
				<div class="bx-sender-caption">Campaign:</div>
				<div class="bx-sender-value">

					<select name="CAMPAIGN_ID" class="bx-sender-letter-form-control bx-sender-letter-field-select">
						<?
						foreach ($arResult['CAMPAIGNS'] as $siteName => $campaigns):
							if (count($arResult['CAMPAIGNS']) > 1)
							{
								?><optgroup label="<?=htmlspecialcharsbx($siteName)?>"><?
							}

							foreach ($campaigns as $campaign):
								?>
								<option value="<?=htmlspecialcharsbx($campaign['ID'])?>" <?=($campaign['SELECTED'] ? 'selected' : '')?>>
									<?=htmlspecialcharsbx($campaign['NAME'])?>
								</option>
								<?
							endforeach;
		...
		<div data-role="letter-buttons" style="<?=($arResult['SHOW_TEMPLATE_SELECTOR'] ? 'display: none;' : '')?>">
			...
			$APPLICATION->IncludeComponent(
				...
				"",
				array(
					'CHECKBOX' => $arResult['CAN_SAVE_AS_TEMPLATE'] ?
						array(
