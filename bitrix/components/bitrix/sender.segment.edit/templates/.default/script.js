;(function ()
{
	...
	{
		return (this.ui.resultView && this.context.getAttribute('data-result-viewable') === 'Y');
	};
	...
	Item.prototype.setCount = function (count)
	{
		...
		Helper.changeDisplay(this.ui.counter, this.counters.length <= 0);
		Helper.changeDisplay(this.ui.countInfo.previousElementSibling, this.counters.length > 0);
	};
	...
	{
		var name = this.manager.contactTileNameTemplate;
		name = name.replace('%count%', listData.COUNT || 0);
