	...
	elseif($_GET["mode"] == "query" || $_POST["mode"] == "query")
	{
		...
		if (!$bCrmMode)
		{
			...
			if(strlen($arParams["EXPORT_FINAL_ORDERS"])>0)
			{
                ...
                $arStatusToExport = array("N"); // Выгружать только со статусом - Не обработан
		...
		$arOrdersToFilter = \Bitrix\Sale\Internals\OrderTable::getList(array(
            "select" => array("*"),
            "filter" => $arFilter
        ))->fetchAll();
		...
		// custom:update orders
		foreach ($arOrdersToFilter as $arOrder)
		{
