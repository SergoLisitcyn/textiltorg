$(document).ready(function() {
    ...
    $("#pay_type div").live( "click", function() {
    ...
    $(".custom_select .select_view").click(function() {
        var cartSum = getCartSum();
        if (cartSum >= 3000) {
            PAY_SYSTEMS["3"] = "В кредит или рассрочку";
        } else {
            delete PAY_SYSTEMS[3];
        }

        var data = "";
        $.each(PAY_SYSTEMS, function(index, value) {
            data += '<div data-value="'+index+'">'+value+'</div>';
        });
        $(".custom_select #pay_type").html(data);
    });
    ...
    $(".select_options div").live( "click", function() {
    ...
    $(".select_view").live( "click", function() {
        $(".select_options").removeClass("active");
...
function getCartSum() {
    var sum = $('.header-cart-sum').text();
    return parseFloat(sum.replace(/\s+/g, ''));
}
...
function InitKladr(address_name) {
    ...
    $.ajax({
        ...
        data : {
            contentType: 'city',
            query: REGION_CITY_NAME, //$city.parents(".custom_select").find('.select_view').val(),
            limit: 1,
        }
    }).done(function(data) {
