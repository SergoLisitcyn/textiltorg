...
class Counter
{
	...
	const CACHE_NAME = 'counter_v1'; // 1 month
	...
	public static function get($userId)
	{
		...
		foreach ($counters as $entity)
		{
			...
			{
				$result['TYPE']['ALL'] += $entity['COUNTER'];
				$result['TYPE']['NOTIFY'] += $entity['COUNTER'];
			}
			else if ($entity['MESSAGE_TYPE'] == IM_MESSAGE_PRIVATE)
			{
				$result['TYPE']['ALL'] += $entity['COUNTER'];
				$result['TYPE']['DIALOG'] += $entity['COUNTER'];
				$result['DIALOG'][$entity['PRIVATE_USER_ID']] = $entity['COUNTER'];
			}
			else
			{
				$result['TYPE']['ALL'] += $entity['COUNTER'];
				$result['TYPE']['CHAT'] += $entity['COUNTER'];
				$result['CHAT'][$entity['CHAT_ID']] = $entity['COUNTER'];
			}
		}
