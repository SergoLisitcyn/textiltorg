...
class CIMNotify
{
	...
	public function MarkNotifyRead($id = 0, $setThisAndHigher = false, $appId = 'Bitrix24')
	{
		...
		$chatId = 0;
		$strSql ="
			SELECT M.ID, M.CHAT_ID, M.NOTIFY_TAG 
			FROM b_im_relation R
		...
		if ($arRes = $dbRes->Fetch())
		{
			if ($setThisAndHigher)
			{
				...
				self::SetLastId($chatId, $this->user_id, $id);
			}
			else
			{
				$id = $arRes['ID'];
	...
	public function MarkNotifyReadBySubTag($subTagList = array())
	{
		...
		while ($message = $res->fetch())
		{
			...
			$messagesByUser[$message["USER_ID"]][$message["ID"]] = $message;
		...
		if (!empty($messages))
		{
			...
			{
				$counter = \CIMNotify::GetCounter($message['CHAT_ID']);
				$DB->Query("UPDATE b_im_relation SET COUNTER = {$counter} WHERE CHAT_ID = ".$message['CHAT_ID']);
			}
			...
			foreach ($users as $userId)
			{
				...
				if ($isLoadPull)
				{
					CPullStack::AddByUser($userId, Array(
						...
						'params' => Array(
							'idList' => array_keys($messagesByUser[$userId]),
						),
					));
				}
	...
	public function MarkNotifyUnRead($id = 0, $setThisAndHigher = false, $appId = 'Bitrix24')
	{
		...
		if ($id > 0)
		{
			...
			if ($arRes = $dbRes->Fetch())
			{
				...
				if ($counter > 0)
				{
					$status = "STATUS = (case when STATUS = ".IM_STATUS_NOTIFY." then STATUS else ".IM_STATUS_UNREAD." end), ";
				}
				$DB->Query("UPDATE b_im_relation SET {$status} COUNTER = {$counter} WHERE CHAT_ID = ".$chatId);
				
				\Bitrix\Im\Counter::clearCache($this->user_id);
