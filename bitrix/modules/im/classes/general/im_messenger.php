	...
	*/
	...
	public static function Delete($id, $userId = null, $completeDelete = false, $byEvent = false)
	{
		...
		if ($completeDelete || $message['CHAT_ID'] == CIMChat::GetGeneralChatId() && self::IsAdmin())
		{
			...
			$relations = \Bitrix\Im\Chat::getRelation($message['CHAT_ID'], Array(
				'SELECT' => Array('ID' => 'RELATION_ID', 'USER_ID'),
				'WITH_REAL_COUNTERS' => 'Y',
				'SKIP_RELATION_WITH_UNMODIFIED_COUNTERS' => 'Y'
			));
			foreach ($relations as $relation)
			{
				\Bitrix\Im\Model\RelationTable::update($relation['RELATION_ID'], Array('COUNTER' => $relation['COUNTER']));
				\Bitrix\Im\Counter::clearCache($relation['USER_ID']);
			}
			...
			BX.ready(function() {
				BXIM = new BX.IM(BX('bx-notifier-panel'), {
					...
					'webrtc': {
						...
						'phoneCallCardRestApps': ".CUtil::PhpToJSObject(self::GetCallCardRestApps()).",
						'phoneLines': ".CUtil::PhpToJSObject(self::GetTelephonyLines()).",
						'phoneDefaultLineId': '".self::GetDefaultTelephonyLine()."',
						'availableLines': ".CUtil::PhpToJSObject(self::GetTelephonyAvailableLines())."
					},
