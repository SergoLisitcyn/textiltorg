...
class CIMChat
{
	...
	function GetLastMessage($toChatId, $fromUserId = false, $loadExtraData = false, $bTimeZone = true, $limit = true)
	{
		...
		if (is_array($loadExtraData) || is_bool($loadExtraData) && $loadExtraData == true)
		{
			...
			if (
				isset($arChat['chat'][$toChatId]) && $arChat['chat'][$toChatId]['messageType'] == IM_MESSAGE_OPEN 
				|| isset($arChat['chat'][$toChatId]) && $arChat['chat'][$toChatId]['messageType'] == IM_MESSAGE_OPEN_LINE 
				|| in_array($fromUserId, $arChat['userInChat'][$toChatId])
			)
			{
	...
	public static function SetUnreadCounter($userId)
	{
		...
		$sqlCounter = "SELECT COUNT(M.ID) as CNT
						FROM b_im_message M
						INNER JOIN b_im_relation R1 ON M.ID > R1.LAST_ID AND M.CHAT_ID = R1.CHAT_ID AND R1.MESSAGE_TYPE IN ('".IM_MESSAGE_OPEN."','".IM_MESSAGE_CHAT."','".IM_MESSAGE_OPEN_LINE."') AND R1.STATUS < ".IM_STATUS_READ."
