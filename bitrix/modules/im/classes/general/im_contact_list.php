...
class CAllIMContactList
{
	...
	function GetList($arParams = Array())
	{
		...
		if ($bLoadUsers)
		{
			...
			if($bCLCacheEnable && $obCLCache->InitCache($ttl, $cache_id, $cache_dir))
			{
				...
				foreach ($arUsers as $userId => $value)
				{
					...
					$arUsers[$userId]['lastActivityDate'] = isset($arOnline['users'][$userId])? $arOnline['users'][$userId]['lastActivityDate']: 0;
					$arUsers[$userId]['isAbsent'] = self::formatAbsentResult($userId);
			...
			else
			{
				...
				while ($arUser = $orm->fetch())
				{
					...
					if (!$skipUser)
					{
						...
						$arUser['IDLE'] = $arUser['IDLE'] instanceof \Bitrix\Main\Type\DateTime? $arUser['IDLE']->getTimestamp(): 0;
						$arUser['MOBILE_LAST_DATE'] = $arUser['MOBILE_LAST_DATE'] instanceof \Bitrix\Main\Type\DateTime? $arUser['MOBILE_LAST_DATE']->getTimestamp(): 0;
						$arUser['LAST_ACTIVITY_DATE'] = $arUser['LAST_ACTIVITY_DATE'] instanceof \Bitrix\Main\Type\DateTime? $arUser['LAST_ACTIVITY_DATE']->getTimestamp(): 0;
						...
						$arUsers[$arUser["ID"]] = Array(
							...
							'externalAuthId' => $arUser['EXTERNAL_AUTH_ID']? $arUser['EXTERNAL_AUTH_ID']: 'default',
							'status' => $arUser['STATUS'],
							'idle' => $arUser['IDLE'],
							'lastActivityDate' => $arUser['LAST_ACTIVITY_DATE'],
							'mobileLastDate' => $arUser['MOBILE_LAST_DATE'],
							'isAbsent' => self::formatAbsentResult($arUser["ID"]),
						);
		...
		$arContactList = Array('users' => $arUsers, 'groups' => $arGroups, 'chats' => $arChats['chat'], 'phones' => $arPhones, 'userInGroup' => $arUserInGroup);
			...
			if($obCache->InitCache($cache_ttl, $cache_id, $cache_dir))
			{
				...
				foreach ($arCacheResult['users'] as $userId => $value)
				{
					if ($showOnline)
					{
						...
						$arCacheResult['users'][$userId]['lastActivityDate'] = isset($arOnline['users'][$userId])? $arOnline['users'][$userId]['lastActivityDate']: 0;
						$arCacheResult['users'][$userId]['isAbsent'] = self::formatAbsentResult($userId);
					}
	...
	public static function SetRecent($arParams)
	{
		...
		if (isset($arParams['CHAT_TYPE']) && in_array($arParams['CHAT_TYPE'], Array(IM_MESSAGE_OPEN, IM_MESSAGE_CHAT, IM_MESSAGE_OPEN_LINE)))
		{
		...
		$isChat = in_array($chatType, Array(IM_MESSAGE_OPEN, IM_MESSAGE_CHAT, IM_MESSAGE_OPEN_LINE));
	...
	public static function GetRecentList($arParams = Array())
	{
		...
		$cache_ttl = 2592000;
		$cache_id = 'im_recent_v12_'.$userId.'_'.$bColorEnabled.'_'.$bOpenChatEnabled;
		...
		if($obCache->InitCache($cache_ttl, $cache_id, $cache_dir))
		{
			...
			foreach ($arRecent as $key => $value)
			{
				...
				if (isset($arOnline['users'][$value['USER']['id']]))
				{
					...
					$arRecent[$key]['USER']['mobileLastDate'] = $arOnline['users'][$value['USER']['id']]['mobileLastDate'];
					$arRecent[$key]['USER']['lastActivityDate'] = $arOnline['users'][$value['USER']['id']]['lastActivityDate'];
					$arRecent[$key]['USER']['isAbsent'] = self::formatAbsentResult($value['USER']['id']);
				}
			}
		}
		else
		{
			...
			while ($arRes = $dbRes->GetNext(true, false))
			{
				...
				if ($arRes['ITEM_TYPE'] == IM_MESSAGE_PRIVATE)
				{
					...
					$arRes['IDLE'] = intval($arRes['IDLE']);
					$arRes['MOBILE_LAST_DATE'] = intval($arRes['MOBILE_LAST_DATE']);
					$arRes['LAST_ACTIVITY_DATE'] = intval($arRes['LAST_ACTIVITY_DATE']);
					...
					$item['USER'] = Array(
						...
						'externalAuthId' => $arRes['EXTERNAL_AUTH_ID']? $arRes['EXTERNAL_AUTH_ID']: 'default',
						'status' => $arRes['STATUS'],
						'idle' => $arRes['IDLE'],
						'lastActivityDate' => $arRes['LAST_ACTIVITY_DATE'],
						'mobileLastDate' => $arRes['MOBILE_LAST_DATE'],
						'isAbsent' => self::formatAbsentResult($arRes["ITEM_ID"]),
					);
				}
				else
				{
	...
	private static function formatAbsentResult($userId)
	{
		...
		return $result? date('c', $result['DATE_TO_TS']+86400): false;
	}
