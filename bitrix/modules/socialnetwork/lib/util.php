...
class Util
{
	...
	/**
	 * Returns true if video message button should be lighted.
	 *
	 * @return bool
	 */
	public static function isShowVideoMessageSpotLight()
	{
		$hintOptionName = 'videomessage_button_spotlight_enable';

		$creationDate = \COption::getOptionInt("main", "~controller_date_create", 0);

		if ($creationDate === 0)
		{
			return \CUserOptions::getOption("socialnetwork", $hintOptionName, true);
		}

		return
			$creationDate < mktime(0, 0, 0, 10, 10, 2017) &&
			time() - $creationDate > 86400 * 14 &&
			\CUserOptions::getOption("socialnetwork", $hintOptionName, true)
			;
	}
}
?>