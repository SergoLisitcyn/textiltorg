...
BX.VideoRecorder = {
	...
	popupConfirm: null,
	constraints: {audio: {}, video: {facingMode: "user"}},
	analyserNode: null,
	activeFormID: null,
	chunks: [],
	bindedForms: [],
	...
	bindEvents: function(handler)
	{
		...
		if(BX('video-message-lighted-button') && !BX.VideoRecorder.spotLightShown && BX.SpotLight && BX.localStorage.get('videoMessageButtonSpotLightShown') !== 'shown')
		{
			BX.VideoRecorder.spotLightShown = true;
			setTimeout(function(){
				var spotlight = new BX.SpotLight({
					top: 10,
					left: 40,
					renderTo: BX('video-message-lighted-button'),
					content: BX.message('BLOG_VIDEO_RECORD_SPOTLIGHT_MESSAGE')
				});
				spotlight.show();
				BX.addCustomEvent(spotlight, "spotLightOk", function(node){
					BX.localStorage.set('videoMessageButtonSpotLightShown', 'shown', 1209600);
				});
			}, 500);
		}
	...
	showLayout: function()
	{
		...
		BX.VideoRecorder.layout = BX.create('div', {props: {className: 'bx-videomessage-video-wrapper'}, children: [
			...
			BX.VideoRecorder.videoOverlay = BX.create('div', {props: {className: 'bx-videomessage-video-overlay'}, style: {height: BX.VideoRecorder.getScreenHeight() + 'px'},  children: [
				BX.VideoRecorder.startTimer = BX.create('div', {props: {className: 'bx-videomessage-video-starttimer'}, style: {display: 'none'}, text: '5'})
			]}),
			BX.VideoRecorder.videoContainer = BX.create('div', {props: {className: 'bx-videomessage-video-container'}, style: {height: BX.VideoRecorder.getScreenHeight() + 'px'}, children: [
				BX.VideoRecorder.videoWrap = BX.create('div', {props: {className: 'bx-videomessage-video-wrap'}, style: {height: BX.VideoRecorder.getScreenHeight() + 'px'}, children: [
					...
					BX.VideoRecorder.panel = BX.create('div', {props: {className: 'bx-videomessage-panel'}, children: [
						...
						BX.VideoRecorder.recordMark = BX.create('div', {props: {className: 'bx-videomessage-record'}, style: {display: 'none'}, text: BX.message('BLOG_VIDEO_RECORD_IN_PROGRESS_LABEL')}),
	...
	beforeRecord: function()
	{
		...
		BX.VideoRecorder.startTimer.style.display = 'block';
		...
		var nextSecond = function()
		{
			...
			BX.VideoRecorder.startTimeout = setTimeout(function()
			{
				...
				{
					BX.VideoRecorder.startTimer.style.display = 'none';
	...
	startRecord: function()
	{
		...
		BX.VideoRecorder.recorder.start();
	...
	resize: function()
	{
		...
		var videoOffset = 0 - (BX.VideoRecorder.outputVideo.clientHeight - height)/2;
		BX.VideoRecorder.outputVideo.style.marginTop = videoOffset + 'px';
		BX.VideoRecorder.videoWrap.style.width = BX.VideoRecorder.outputVideo.clientWidth + 'px';
		BX.VideoRecorder.panel.style.width = BX.VideoRecorder.outputVideo.clientWidth + 'px';
