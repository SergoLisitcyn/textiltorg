...
class BitrixMarket
{
    ...
    private $arRegionPrices = array(
		...
		"BY"    => 11,
		"BYN"   => 11,
		"NSK" 	=> 12 //Новосибирская область && Новосибирск
    );
    ...
    public function GetSubCategory($arrTargetCategory)
    {
    ...
    public function GetSectionsXml($excludeRootCat = false, $arSubCategory = array())
    {
        $arTargetCategory = array();
        if (!empty($arSubCategory)) {
            foreach ($arSubCategory as $i => $id) {
                $nav = CIBlockSection::GetNavChain(false, $id, array("ID", "IBLOCK_SECTION_ID", "NAME"));
                while($arSectionPath = $nav->ExtractFields()){
                    if ($excludeRootCat) {
                        if ($arSectionPath["IBLOCK_SECTION_ID"] != null) {
                            $arTargetCategory[] = $arSectionPath["ID"];
                        }
                    } else {
                        $arTargetCategory[] = ($arSectionPath["ID"]) ? $arSectionPath["ID"] : $arSectionPath["IBLOCK_SECTION_ID"];
                    }
                }
            }
        }
        ...
        while ($arRow = $dbResult->GetNext()) {
            if (!empty($arTargetCategory)) {
                if (in_array($arRow["ID"], $arTargetCategory)) {
                    $this->arSections[$arRow["ID"]] = $arRow;
                    $this->arSectionsId[] = $arRow["ID"];
                    $this->arSectionsYmId[$arRow["ID"]] = ($arRow["UF_YM_ID"]) ? $arRow["UF_YM_ID"] : $arRow["ID"];
        ...
        foreach ($arSections as $arSection) {
            $arXmlCategory = array(
                "@id" => ($arSection["UF_YM_ID"]) ? $arSection["UF_YM_ID"] : $arSection["ID"],
            );
    ...
    public function GetElementsXml2($isNotProps = false, $noExportMarket = false, $isFilterSection = true)
    {
        $arResult = array();
        ...
        if ($isNotProps) {
            ...
            $this->arElements = $this->GetElements2($this->arElements, $arProps, array(), $noExportMarket, $isFilterSection);
        } else {
        ...
        foreach ($this->arElements as $arItem) {
            ...
            /*
             * 1. В случае если устанавливаем в карточке товара «Бесплатно»  то в YML файле цена устанавливается 0 независимо от цены товара.
             * 2. В случае если устанавливаем «Бесплатно по РФ» то в YML файле цена устанавливается 0 независимо от цены товара.
             * 3. В случае если устанавливаем «Платно» то в YML файле устанавливается платная доставка 300 рублей в карточке товара так же для всех регионов устанавливается платно 300 как для домашних регионов так и для остальных
             * 4. Условие которое устанавливает цену в зависимости от стоимости товара необходимо отключить.
            */
            // Цена доставки по региону: если есть - установить, иначе дефолтная
            ...
            // Если цена товара больше лимита установить цену доставки 0 - отключено
            //$deliveryCost = (intval($arItem["CATALOG_PRICE_" . $this->regionPriceId]) >= self::PRICE_FREE_DELIVERY) ? "0" : $priceDelivery;

            // Установим цену для региона по умолчанию
            $deliveryCost = (empty($this->arPricesDelivery[$this->regionCode])) ? self::DEFAULT_PRICE_DELIVERY : $this->arPricesDelivery[$this->regionCode];

            if (!empty($arItem["PROPERTIES"]["DELIVERY"]["VALUE"]))
            {
                switch($arItem["PROPERTIES"]["DELIVERY"]["VALUE"][0])
                {
                    case "Бесплатная":
                        $deliveryCost = "0";
                        break;
                    case "Бесплатная по РФ":
                        $deliveryCost = "0";
                        break;
                    case "Платная":
                        break;
    ...
    public function GetElementsXmlTmall($isNotProps = false, $noExportMarket = false, $isFilterSection = true, $targetProducts)
    {
        $arResult = array();
        ...
        if ($isNotProps) {
            ...
            $this->arElements = $this->GetElements($this->arElements, $arProps, $targetProducts, $noExportMarket, $isFilterSection);
        } else {
            $arContainerProps = $this->GetContainerProperties(array('EAN13'));
        ...
        foreach ($this->arElements as $arItem) {
            if (!$isNotProps) {
                $arFormatProps = array();
                $arFormatComplect = array();

                // Комплектация
                if (!empty($arItem["PROPERTIES"]["EQUIPMENT"]["VALUE"])) {
                    $arFormatComplect[] = $this->GetEquipmentsXML($arItem["PROPERTIES"]["EQUIPMENT"]["VALUE"]);
                }
            ...
            if (!$isNotProps) {
                $arFormatRow["vendorCode"] = $arFormatComplect;
                $arFormatRow["param"] = $arFormatProps;
    ...
    public function GetElementsXml3($isNotProps = false, $noExportMarket = false, $isFilterSection = true)
    {
        $arResult = array();
        ...
        if ($isNotProps) {
            ...
            $this->arElements = $this->GetElements($this->arElements, $arProps, array(), $noExportMarket, $isFilterSection);
        } else {
            $arContainerProps = $this->GetContainerProperties();
        ...
        foreach ($this->arElements as $arItem) {
            if (!$isNotProps) {
                ...
                if (!empty($arItem["PROPERTIES"]["EQUIPMENT"]["VALUE"])) {
                    $arFormatProps[] = $this->GetEquipmentsXML($arItem["PROPERTIES"]["EQUIPMENT"]["VALUE"]);
                }
            }
            ...
            if (!$isNotProps) {
                $arFormatRow["vendorCode"] = $arFormatProps;
            }
    ...
    public function GetElementsXml($isNotProps = false, $noExportMarket = false, $isFilterSection = true, $arExtFilter = array())
    {
        $arResult = array();
        ...
        if ($isNotProps) {
            ...
            $this->arElements = $this->GetElements($this->arElements, $arProps, $arExtFilter, $noExportMarket, $isFilterSection);
        } else {
            $arContainerProps = $this->GetContainerProperties();
        ...
        foreach ($this->arElements as $arItem) {
			...
			*/
            ...
            $arFormatRow += array(
                ...
                "name" => $arItem["NAME"],
                //"name" => (substr($arItem["NAME"], -3) == ' во')? substr($arItem["NAME"], 0, -3) : $arItem["NAME"],
            );
    ...
    public function GetElementsXmlGoods($isNotProps, $arVendorCodes)
    {
        ...
        if ($isNotProps) {
            ...
            $arFilterExt = array("PROPERTY_VENDOR_CODE" => $arVendorCodes, ">CATALOG_QUANTITY" => 0);

            $this->arElements = $this->GetElements($this->arElements, $arProps, $arFilterExt);
        } else {
            $arContainerProps = $this->GetContainerProperties();
            $arFilterExt = array(">CATALOG_QUANTITY" => 0);
    ...
    public function GetElementsXmlAllBiz($arrTargetCategory)
    {
        ...
        foreach ($this->arElements as $arItem) {
            ...
            $tmpPrice = ($price / 100) * 30;

            $arFormatRow += array(
                "url" => $this->GetOfferUrl($arItem["DETAIL_PAGE_URL"]),
                "price" => $price,
                "oldprice" => $price + $tmpPrice,
                "currencyId" => $arItem["CATALOG_CURRENCY_" . $this->regionPriceId],
                "categoryId" => $this->arSectionsYmId[$arItem["IBLOCK_SECTION_ID"]],
                "picture" => $arPictures[$arItem["DETAIL_PICTURE"]],
                "delivery" => "true",
...
//        );
    ...
    public function GetElementsXmlGoogleMS($arrTargetCategory, $arrTargetProducts, $arOutOfStock = array())
    {
        $arrResultCategory = $this->GetSubCategory($arrTargetCategory);
        ...
        if (empty($arrTargetProducts)) {
            $arExtFilter = array(
                "SECTION_ID" => $arrResultCategory,
                "!PROPERTY_EAN13" => false
            );
        } else {
            $arExtFilter = array(
                "SECTION_ID" => $arrResultCategory,
                "XML_ID" => $arrTargetProducts,
                "!PROPERTY_EAN13" => false
            );
        }
        ...
        foreach ($this->arElements as $arItem) {
            ...
            $priceDelivery = (empty($this->arPricesDelivery[$this->regionCode])) ? self::DEFAULT_PRICE_DELIVERY : $this->arPricesDelivery[$this->regionCode];
            $deliveryCost = (intval($arItem["CATALOG_PRICE_" . $this->regionPriceId]) >= self::PRICE_FREE_DELIVERY) ? "0" : $priceDelivery;
            $availability = (intval($arItem["CATALOG_PRICE_" . $this->regionPriceId]) > 0) ? 'in stock' : 'out of stock';

            if (in_array($arItem["ID"], $arOutOfStock)) {
            ...
            $arFormatRow = array(
                ...
                "description" => self::FormatTextDesc($arItem["PREVIEW_TEXT"]),
                "g:availability" => $availability, //(intval($arItem["CATALOG_QUANTITY"])) ? "in stock" : "out of stock",
                "g:id" => $arItem["ID"],
                "g:condition" => "new",
                ...
                "g:gtin" => $arItem["PROPERTIES"]["EAN13"]["VALUE"]
            );
    ...
    public function GetElementsForAvito($noExportMarket = false)
    {
        ...
        foreach ($this->arElements as $arItem) {
            ...
            $description .= "\n\n Внимание! На каждый товар приобретенный в нашем магазине Вы получаете все необходимые документы, включая чек и официальный гарантийный талон. Есть доставка по РФ. Весь товар новый! В магазине действуют дополнительные скидки и акции, посетите сайт магазина Текстильторг, что бы узнать больше.";
...
//        return "true";
    ...
    private function GetElements2($arResult = array(), $arProps = array(), $arFilterExt = array(), $noExportMarket = false, $isFilterSection = true)
    {
        if (empty($this->arSectionsId) && $isFilterSection)
            return array();
...
//        }
    ...
    private function GetContainerProperties($arSystemPropsExt = array())
    {
        $arResult = array();
        ...
        if (!empty($arSystemPropsExt)) {
            $arSystemProps = array_merge($arSystemProps, $arSystemPropsExt);
        }
