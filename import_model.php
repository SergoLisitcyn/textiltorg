...
$url = 'http://textiletorg.ru/sync_base/get_model_data.php';

$response = curlGet($url, array());
if ($response['code'] != 200) {
    var_dump('Ошибка связи с удаленным сервером');
    die();
}

$skk_model_data = json_decode($response['response'], true);
...
while ($arElement = $rsElements->Fetch())
{
    ...
    /*    $idElement = $arElement["ID"];
        ...
        $arFields = array(
            "CATEGORY_TYPE" =>  $skk_model_data[$arElement["PROPERTY_VENDOR_CODE_VALUE"]]["CATEGORY_TYPE"],
            "MODEL" =>          $skk_model_data[$arElement["PROPERTY_VENDOR_CODE_VALUE"]]["MODEL"],
            "MODEL_NAME" =>     $skk_model_data[$arElement["PROPERTY_VENDOR_CODE_VALUE"]]["MODEL_NAME"],
            "SHORT_NAME" =>     $skk_model_data[$arElement["PROPERTY_VENDOR_CODE_VALUE"]]["SHORT_NAME"]
        );
